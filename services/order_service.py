...
def Bottom_pickup(Bottom: str): #Route dependend on cover color
...
def Fuse_pickup(top_fuse: bool, bottom_fuse: bool): #Route dependend on amount and placement of fuse
...
def Hole_drill(top_holes: bool, bottom_holes: bool): #Route dependend on amount of holes
    """Drills holes for the bottom cover, 1: top holes, 2: bottom holes, 3: all four holes."""
    if top_holes and bottom_holes:
...
def From_fuse_to_assembly(top_fuse: bool, bottom_fuse: bool):
    """Takes the path from fuse to assembly"""
    # No path planning yet
    robot.setPoseFrame(RDK.Item("Universal frame"))
    if top_fuse or bottom_fuse:
        robot.MoveL(RDK.Item("BetweenFuseAndAssembly"))
